...
impl ClashManager {
    ...
    fn cleanup_orphan_processes() -> Result<(), String> {
        #[cfg(windows)]
        {
            // 使用 tasklist 查找 TorBoxCore.exe 进程
            let output = Command::new("tasklist")
                .args(["/FI", "IMAGENAME eq TorBoxCore.exe", "/FO", "CSV", "/NH"])
                .stdout(Stdio::piped())
                .output()
            ...
            // 解析 CSV 输出格式："TorBoxCore.exe","1234","Console","1","12,345 K"
            for line in stdout.lines() {
                if line.contains("TorBoxCore.exe") {
                    // 提取 PID（第二列）
                    let parts: Vec<&str> = line.split(',').collect();
                    if parts.len() >= 2 {
                        let pid_str = parts[1].trim_matches('"').trim();
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            log::warn!("检测到孤立的 TorBoxCore.exe 进程 PID={}，正在清理", pid);
        ...
        #[cfg(unix)]
        {
            ...
            for line in stdout.lines() {
                if let Ok(pid) = line.trim().parse::<u32>() {
                    log::warn!("检测到孤立的 TorBoxCore 进程 PID={}，正在清理", pid);
