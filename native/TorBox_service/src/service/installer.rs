...
const SERVICE_NAME: &str = "TorBoxService";
...
#[cfg(windows)]
const SERVICE_DISPLAY_NAME: &str = "TorBox Service";
#[cfg(windows)]
const SERVICE_DESCRIPTION: &str = "TorBox 后台服务，用于管理 Clash 核心和提供系统级 TUN 支持";

#[cfg(windows)]
pub fn install_service() -> Result<()> {
    println!("正在安装 TorBox Service...");
...
#[cfg(target_os = "linux")]
const SERVICE_FILE: &str = "/etc/systemd/system/TorBoxService.service";
...
#[cfg(target_os = "linux")]
pub fn install_service() -> Result<()> {
    println!("正在安装 TorBox Service (systemd)...");
...
#[cfg(target_os = "linux")]
pub fn uninstall_service() -> Result<()> {
    println!("正在卸载 TorBox Service (systemd)...");
...
#[cfg(target_os = "macos")]
const SERVICE_LABEL: &str = "com.TorBox.service";
#[cfg(target_os = "macos")]
const SERVICE_PLIST_PATH: &str = "/Library/LaunchDaemons/com.TorBox.service.plist";
...
<dict>
    ...
    <string>/var/log/TorBoxService.log</string>
    <key>StandardErrorPath</key>
    <string>/var/log/TorBoxService-error.log</string>
</dict>
</plist>"#,
...
#[cfg(target_os = "macos")]
pub fn install_service() -> Result<()> {
    println!("正在安装 TorBox Service (launchd)...");
    ...
    // 创建临时文件（使用唯一路径避免冲突）
    let temp_plist = "/tmp/TorBoxService-install.plist";
...
#[cfg(target_os = "macos")]
pub fn uninstall_service() -> Result<()> {
    println!("正在卸载 TorBox Service (launchd)...");
...
// 获取服务私有目录路径（AppData/Roaming/TorBox/service）
#[cfg(any(windows, target_os = "linux", target_os = "macos"))]
...
fn get_service_private_binary() -> Result<std::path::PathBuf> {
    Ok(get_service_private_dir()?.join("TorBoxService.exe"))
}

#[cfg(not(windows))]
fn get_service_private_binary() -> Result<std::path::PathBuf> {
    Ok(get_service_private_dir()?.join("TorBoxService"))
}
