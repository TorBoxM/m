...
impl PathService {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        // 服务二进制文件名
        #[cfg(target_os = "windows")]
        let service_exe_name = "TorBoxService.exe";
        #[cfg(not(target_os = "windows"))]
        let service_exe_name = "TorBoxService";
        ...
        let tasks_dir = {
            ...
            PathBuf::from(appdata).join("TorBox").join("tasks")
        };
    ...
    fn get_service_private_dir() -> Result<PathBuf, String> {
        #[cfg(target_os = "windows")]
        {
            let appdata = std::env::var("APPDATA")
                .map_err(|e| format!("无法获取 APPDATA 环境变量：{}", e))?;
            Ok(PathBuf::from(appdata).join("TorBox").join("service"))
        }
    ...
    fn fallback() -> Self {
        ...
        Self {
            ...
            service_private_binary: current_dir.join("service").join("TorBoxService"),
...
pub fn service_private_binary() -> PathBuf {
    PATH_SERVICE
        ...
        .unwrap_or_else(|_| PathBuf::from("TorBoxService"))
}
